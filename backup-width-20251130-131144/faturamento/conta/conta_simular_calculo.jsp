...
<logic:notEqual name="indicadorTarifaCategoria"
	...
	<!--
		function removerCategoria(idCategoria) {
			...
			form.action = "/gsan/removerSelecaoCategoriaAction.do?idCategoria="
					+ idCategoria + "&mapeamento=exibirSimularCalculoConta";
				...
				if (form.elements[i].type == "text"
					...
					case "categoria":
						if (form.elements[i].value.length < 1) {
							...
							form.elements[i].focus();
							camposValidos = false;
						} else if (!testarCampoValorZero(form.elements[i],
...
<logic:equal name="indicadorTarifaCategoria"
	...
	<!--
		function removerSubcategoria(idSubcategoria) {
			...
			form.action = "/gsan/removerSelecaoCategoriaAction.do?idSubcategoria="
					+ idSubcategoria + "&mapeamento=exibirSimularCalculoConta";
...
<!--
	...
	function habilitacaoCamposEsgoto(listBoxEsgoto, consumoFaturadoEsgoto,
			percentualEsgoto, consumoFaturadoPoco) {
	...
	function copiarDadosLigacaoEsgoto(listBoxEsgoto, consumoFaturadoEsgoto,
			percentualEsgoto) {
		...
		if (indicadorFaturamento.length > 0 && indicadorFaturamento == ATIVO) {
			...
			form.percentualEsgoto.value = document
					.getElementById("PERCENTUAL_ESGOTO").value;
		}
	}

	function validarForm(form, listBoxAgua, listBoxEsgoto, consumoFaturadoAgua,
			consumoFaturadoEsgoto, percentualEsgoto, percetualColeta) {
		...
		if (validateSimularCalculoContaActionForm(form)) {
			...
			if (!verificaAnoMesMensagemPersonalizada(form.mesAnoReferencia,
					'Mês e Ano da Conta inválidos.')) {
				valido = false;
			...
			} else if (consumoFaturadoAgua.value.length > 0
					&& consumoFaturadoAgua.value != 0
					&& consumoFaturadoAgua.value <= listBoxAgua.options[listBoxAgua.selectedIndex].name) {
				valido = false;
				alert("Consumo informado menor que consumo mínimo para situação da ligação de água, valor tem que ser maior que "
						+ listBoxAgua.options[listBoxAgua.selectedIndex].name);
				consumoFaturadoAgua.focus();
			} else if ((indicadorFaturamentoEsgoto.length > 0 && indicadorFaturamentoEsgoto == ATIVO)
					&& consumoFaturadoEsgoto.value.length < 1
					&& percetualColeta.value.length < 1) {
				valido = false;
				alert("Informe Consumo de Esgoto ou Percentual de Coleta.");
				consumoFaturadoEsgoto.focus();
			} else if (consumoFaturadoEsgoto.value.length > 0
					&& consumoFaturadoEsgoto.value != 0
					&& consumoFaturadoEsgoto.value <= listBoxEsgoto.options[listBoxEsgoto.selectedIndex].name) {
				valido = false;
				alert("Volume informado menor que volume mínimo para situação da ligação de esgoto, valor tem que ser maior que "
						+ listBoxesgoto.name);
				consumoFaturadoEsgoto.focus();
			} else if ((indicadorFaturamentoEsgoto.length > 0 && indicadorFaturamentoEsgoto == ATIVO)
				...
				alert("Informe Categorias e Economias.");
					...
					&& consumoFaturadoEsgoto.value.length < 1
					&& form.percentualColeta.value.length < 1) {
				//form.percentualColeta.value.length < 1){
	...
	function habilitaVolumeEsgoto() {
		...
		if (form.consumoFaturadoPoco.value != ''
				&& form.percentualColeta.value != '') {
			habilitacaoConsumoEsgoto(form.ligacaoEsgotoSituacaoID,
					form.consumoFaturadoEsgoto, form.percentualColeta,
					form.consumoFaturadoAgua, form.consumoFaturadoPoco);
		}

		if ((form.consumoFaturadoPoco.value != '' && form.consumoFaturadoPoco.readOnly == false)
				|| (form.percentualColeta.value != '' && form.percentualColeta.readOnly == false)) {
	...
	function habilitacaoConsumoEsgoto(listBoxEsgoto, consumoFaturadoEsgoto,
			percentualColeta, consumoAgua, consumoPoco) {
		var form = document.forms[0];
		...
		if (indicadorFaturamento.length > 0 && indicadorFaturamento == ATIVO) {
			...
			if (percentualColeta.value != "") {
				...
				if (consumoAgua.value != "" || consumoPoco.value != "") {
					...
					percentual = percentual.replace(/,/g, '.');
					consumoFaturadoEsgoto.value = Math.round(consumo
							* (percentual / 100));
				}

				if (((percentualColeta.value != '' && percentualColeta.readOnly == false) || (consumoPoco.value != '' && consumoPoco.readOnly == false))
						&& consumoFaturadoEsgoto.value != '') {
					consumoFaturadoEsgoto.readOnly = true;
				}
			} else {
				if (consumoFaturadoEsgoto.readOnly == false
						&& consumoFaturadoEsgoto.value != "") {
					percentualColeta.readOnly = true;

				}
				if ((consumoPoco.value != '' && consumoPoco.readOnly == false)
						|| (percentualColeta.value != '' && percentualColeta.readOnly == false)) {
					consumoFaturadoEsgoto.readOnly = true;
				} else {
	...
	function habilitacaoConsumoPoco(listBoxEsgoto, consumoFaturadoEsgoto,
			consumoPoco) {
		var form = document.forms[0];
	...
	function habilitacaoPercentualColeta(listBoxEsgoto, consumoFaturadoEsgoto,
			percentualColeta) {
		var form = document.forms[0];
...
<body leftmargin="5" topmargin="5"
	...
	<html:form action="/simularCalculoContaAction" method="post">
		...
		<INPUT TYPE="hidden" ID="PERCENTUAL_ESGOTO"
			value="${requestScope.percentualEsgotoDefault}" />
					...
					<table width="100%" border="0">
						<tr>
							<td width="100%">
								<table width="100%" border="0">
									<tr>
										<td colspan="3">Para simular o c&aacute;lculo da conta,
		...
		<script language="JavaScript">
		<!--
			habilitacaoCamposAgua(document.forms[0].ligacaoAguaSituacaoID,
					document.forms[0].consumoFaturadoAgua);
			habilitacaoCamposEsgoto(document.forms[0].ligacaoEsgotoSituacaoID,
					document.forms[0].consumoFaturadoEsgoto,
					document.forms[0].percentualEsgoto,
					document.forms[0].consumoFaturadoPoco);
		//-->
		</script>
	</html:form>
</body>
