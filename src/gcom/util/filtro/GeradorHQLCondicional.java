...
public class GeradorHQLCondicional {
    ...
    public static Query gerarCondicionalQuery(Filtro filtro, String aliasTabela, String query, Session session)
        ...
        // ===== NOVA MONTAGEM DA QUERY (CORRIGIDA) =====
        String condicional = queryCondicional.toString();

        if (condicional != null && condicional.trim().length() > 0) {
            String condTrim = condicional.trim();

            // Caso especial: só sobrou "where" (nenhuma condição válida foi adicionada)
            if (condTrim.equalsIgnoreCase("where")) {
                condicional = "";
            } else {
                // se terminar com "and" remove
                if (condTrim.toLowerCase().endsWith("and")) {
                    condTrim = condTrim.substring(0, condTrim.length() - 3).trim();
                }

                // garante que começa com "where"
                if (!condTrim.toLowerCase().startsWith("where")) {
                    condTrim = "where " + condTrim;
                }

                condicional = " " + condTrim;
            }
        } else {
            condicional = "";
        }

        condicionalQueryRetorno.setQuery(new StringBuffer(query + condicional));
	...
	private static Query efetuarQuery(CondicionalQuery condicionalQuery, Session session) throws HibernateException {
	    ...
	    // HQL original montada pelo CondicionalQuery
	    String hql = condicionalQuery.getQuery().toString();
